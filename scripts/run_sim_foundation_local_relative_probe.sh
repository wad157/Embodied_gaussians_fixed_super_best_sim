...
OUTPUT_ROOT="${1:-$ROOT_DIR/outputs/sim_foundation_local_relative_super_v1}"
...
METHOD_C="pbd_cotracker_foundation_depth_local_relative_super"
...
if [[ $# -gt 1 ]]; then
    echo "用法：bash scripts/run_sim_foundation_local_relative_probe.sh [新输出目录]" >&2
    exit 2
fi
...
for capability in reconstruction_7to1 future_80to20; do
    for method in pbd "$METHOD_B"; do
        source="$BASELINE_ROOT/$capability/$method/trajectory_metrics.json"
        target="$OUTPUT_ROOT/$capability/$method"
        mkdir -p "$target"
        cp "$source" "$target/trajectory_metrics.json"
    done
done
...
run_c() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" \
        TORCH_EXTENSIONS_DIR="/Media_HDD/jwshan/tmp/torch_extensions_foundation_local_super_gpu${gpu_id}" \
        bash "$ROOT_DIR/scripts/run_sim_reconstruction_headless.sh" \
            ...
            --stiffness-update-mode differentiable_local_relative \
...
"$PYTHON" "$ROOT_DIR/scripts/summarize_sim_relative_stiffness_probe.py" \
    ...
    --method-c "$METHOD_C" \
    --label-c "C-L：B + SUPER式局部零均值H1/H3/H5刚度更新"
"$PYTHON" "$ROOT_DIR/scripts/summarize_sim_local_relative_diagnostics.py" \
    --evaluation-root "$OUTPUT_ROOT" \
    --method "$METHOD_C"
find "$OUTPUT_ROOT" -type f ! -name SHA256SUMS -print0 | sort -z | xargs -0 sha256sum \
    >"$OUTPUT_ROOT/SHA256SUMS"
echo "完成：$OUTPUT_ROOT/trajectory_probe_comparison.md"
