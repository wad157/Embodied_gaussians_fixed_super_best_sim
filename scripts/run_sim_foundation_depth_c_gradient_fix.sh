...
OUTPUT_ROOT="${1:-$ROOT_DIR/outputs/sim_foundation_depth_c_gradient_fix_v2}"
...
if [[ $# -gt 1 ]]; then
    echo "用法：bash scripts/run_sim_foundation_depth_c_gradient_fix.sh [新输出目录]" >&2
    exit 2
fi
...
for capability in reconstruction_7to1 future_80to20; do
    for method in pbd "$METHOD_B"; do
        source="$BASELINE_ROOT/$capability/$method"
        target="$OUTPUT_ROOT/$capability/$method"
        mkdir -p "$target"
        cp "$source/trajectory_metrics.json" "$target/trajectory_metrics.json"
        cp "$source/render_metrics.json" "$target/render_metrics.json"
    done
done
...
run_c() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" \
        ...
        bash "$ROOT_DIR/scripts/run_sim_reconstruction_headless.sh" \
            ...
            --evaluation-label "${capability}_${METHOD_C}_gradient_fix" \
