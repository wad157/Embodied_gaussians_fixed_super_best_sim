...
TRACE_PY="$ROOT_DIR/scripts/run_trace_baseline_python.sh"
EVAL_PY="${EVAL_ENV_PREFIX:-/Media_HDD/jwshan/conda_envs/eg_codex}/bin/python"
GPU_ID="${SIM_GPU_ID:-0}"

if [[ $# -ne 4 ]]; then
    echo "用法：bash scripts/run_trace_sim_baseline_once.sh <sim01|sim02|sim03> <repeat_01> <seed> <输出目录>" >&2
    exit 2
fi
DATASET_KEY="$1"
REPEAT_ID="$2"
...
if [[ -e "$OUTPUT_ROOT" ]]; then
    echo "ERROR: 拒绝覆盖已有 TRACE 输出：$OUTPUT_ROOT" >&2
    exit 1
fi
...
"$EVAL_PY" "$ROOT_DIR/scripts/audit_trace_sim_protocol.py" \
...
CUDA_VISIBLE_DEVICES="$GPU_ID" bash "$TRACE_PY" \
    "$ROOT_DIR/baselines/trace_sim/train_sim.py" \
    --dataset-key "$DATASET_KEY" \
    --seed "$SEED" \
    --source_path "$DATASET" \
    ...
    --iterations 40000 \
    --sim-downsample 2 \
    --sim-init-points 50000 \
    >>"$LOG" 2>&1

CUDA_VISIBLE_DEVICES="$GPU_ID" bash "$TRACE_PY" \
    "$ROOT_DIR/baselines/trace_sim/export_sim.py" \
    --dataset-key "$DATASET_KEY" \
    --source-path "$DATASET" \
