...
BINDINGS="$ASSET_ROOT/bindings.npz"
OBSERVATIONS="$ASSET_ROOT/observations.npz"
...
BASELINE_ROOT="${SIM_BASELINE_ROOT:-$ROOT_DIR/outputs/sim_known_grasp_causal_full_pbd30hz_v1}"
OUTPUT_ROOT="${1:-$ROOT_DIR/outputs/sim_cotracker_gt_depth_eval30_v1}"
...
if [[ $# -gt 1 ]]; then
    echo "用法：bash scripts/run_sim_cotracker_gt_depth_evaluation.sh [新输出目录]" >&2
    exit 2
fi
...
mkdir -p "$OUTPUT_ROOT/logs" "$OUTPUT_ROOT/baseline_30"
export SIM_DATASET="$DATASET"

rescore_baseline() {
    local capability="$1"
    local method="$2"
    local source="$BASELINE_ROOT/$capability/$method"
    local target="$OUTPUT_ROOT/baseline_30/$capability/$method"
    local metric_args=()
    ...
    "$PYTHON" "$ROOT_DIR/scripts/evaluate_sim_trajectory_metrics.py" \
        ...
        "${metric_args[@]}" \
        --output "$target/trajectory_metrics.json"
}

for capability in reconstruction_7to1 future_80to20; do
    rescore_baseline "$capability" pbd
    rescore_baseline "$capability" pbd_visual_residual
done
...
run_case() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" \
        TORCH_EXTENSIONS_DIR="/Media_HDD/jwshan/tmp/torch_extensions_sim_flow_gpu${gpu_id}" \
        bash "$ROOT_DIR/scripts/run_sim_reconstruction_headless.sh" \
            ...
            --stiffness-update-mode differentiable_low_dim \
    ...
    "$PYTHON" "$ROOT_DIR/scripts/compare_sim_learned_stiffness_to_gt.py" \
        ...
        --analysis-frames 0,123,126,132,160,200,240,287,359 \
...
run_lane() {
    local gpu_id="$1"
    local capability="$2"
    run_case "$gpu_id" "$capability" pbd_cotracker_gt_depth off
    run_case "$gpu_id" "$capability" pbd_cotracker_gt_depth_stiffness on
}

set +e
run_lane 0 reconstruction_7to1 \
    >"$OUTPUT_ROOT/logs/reconstruction_gpu0.log" 2>&1 &
reconstruction_pid=$!
run_lane 1 future_80to20 \
