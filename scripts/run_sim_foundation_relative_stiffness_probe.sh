...
OUTPUT_ROOT="${1:-$ROOT_DIR/outputs/sim_foundation_relative_stiffness_probe_v1}"
...
METHOD_C="pbd_cotracker_foundation_depth_global_relative"
...
if [[ $# -gt 1 ]]; then
    echo "用法：bash scripts/run_sim_foundation_relative_stiffness_probe.sh [新输出目录]" >&2
    exit 2
fi
...
run_c() {
    ...
    CUDA_VISIBLE_DEVICES="$gpu_id" \
        TORCH_EXTENSIONS_DIR="/Media_HDD/jwshan/tmp/torch_extensions_foundation_relative_gpu${gpu_id}" \
        bash "$ROOT_DIR/scripts/run_sim_reconstruction_headless.sh" \
            ...
            --stiffness-update-mode differentiable_global_relative \
...
"$PYTHON" "$ROOT_DIR/scripts/summarize_sim_foundation_c_gradient_diagnostics.py" \
    --evaluation-root "$OUTPUT_ROOT" \
    --method "$METHOD_C"
