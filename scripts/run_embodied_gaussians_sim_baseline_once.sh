...
GPU_ID="${SIM_GPU_ID:-0}"
ACTUATION="${EG_ACTUATION:-psm-fk-collision-only}"

if [[ $# -ne 4 ]]; then
    echo "用法：bash scripts/run_embodied_gaussians_sim_baseline_once.sh <sim01|sim02|sim03> <repeat_01|repeat_02|repeat_03> <seed> <输出目录>" >&2
    exit 2
fi
DATASET_KEY="$1"
REPEAT_ID="$2"
...
case "$ACTUATION" in
    shared-prescribed-boundary|psm-fk-collision-only) ;;
    *) echo "ERROR: EG_ACTUATION 非法：$ACTUATION" >&2; exit 2 ;;
esac
...
for required in "$EG_PY" "$DATASET/episode.json"; do
    if [[ ! -e "$required" ]]; then
...
printf '%s\n' \
    ...
    "gpu=$GPU_ID" \
    "actuation=$ACTUATION" \
    'method=embodied_gaussians_soft_paper_reconstruction' \
...
CUDA_VISIBLE_DEVICES="$GPU_ID" "$EG_PY" \
    "$ROOT_DIR/baselines/embodied_gaussians_sim/run_soft.py" \
    --dataset-key "$DATASET_KEY" \
    --dataset "$DATASET" \
    --body "$BODY" \
...
CUDA_VISIBLE_DEVICES="$GPU_ID" "$EG_PY" \
    ...
    --dataset "$DATASET" \
    --body "$BODY" \
    --rollout-dir "$ARTIFACTS" \
    --output "$ARTIFACTS/predicted_trajectories.npz" \
    >>"$LOG" 2>&1
