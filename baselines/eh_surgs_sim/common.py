#!/usr/bin/env python3
"""Leakage-free SIM adapter for the pinned EH-SurGS implementation."""
    ...
    def __init__(self, dataset_root: Path):
        ...
        for name in CAMERAS:
            ...
            self.arrays[name] = np.load(
                str(root / "tissue_masks_packbits.npy"), mmap_mode="r"
            )
...
def load_training_arrays(
    ...
    image = np.asarray(
        Image.open(str(dataset_root / "rgb" / camera / "{:06d}.png".format(frame))).convert("RGB"),
        dtype=np.float32,
    ) / 255.0
    depth = np.asarray(
        np.load(str(depth_root / camera / "{:06d}-depth.npy".format(frame))),
        dtype=np.float32,
...
class SimTrainingData:
    """Load only legal prefix observations; never open evaluation truth."""
    ...
    def __init__(
        ...
        self.seed = int(seed)
        self.depth_root = self.dataset_root / "estimated_depth" / str(self.spec["depth"])
    ...
    def initial_point_cloud(self) -> BasicPointCloud:
        ...
        for camera in self.train_cameras:
            ...
            colors = camera.original_image[:, y, x].permute(1, 0).numpy()
        ...
        selected = rng.choice(
            len(points), size=self.init_points, replace=len(points) < self.init_points
        )
...
class SimScene:
    ...
    def save(self, iteration, stage):
        del stage
        path = os.path.join(
            self.model_path, "point_cloud", "iteration_" + str(iteration)
        )
...
def load_trained_gaussians(model_path: Path, iteration: int, sh_degree: int, hyper):
    ...
    checkpoint = root / "iteration_{}".format(resolved)
    if not checkpoint.is_dir():
        raise FileNotFoundError("找不到 EH-SurGS checkpoint：{}".format(checkpoint))
    ...
    if not mask_path.is_file():
        raise FileNotFoundError("缺少 EH-SurGS 自适应形变掩码：{}".format(mask_path))
    mask = torch.from_numpy(np.load(str(mask_path)).astype(bool)).to("cuda:0")
    if mask.shape != gaussians._deformation_table.shape:
        raise ValueError("形变掩码与 Gaussian 数量不一致")
    return gaussians, mask, resolved


def deformation_selection(gaussians, learned_mask: torch.Tensor) -> torch.Tensor:
    selected = gaussians._deformation_table.clone()
    selected[: min(4000, len(selected))] = False
    return selected | learned_mask


def deformed_centers(
    gaussians, learned_mask: torch.Tensor, normalized_time: float
) -> torch.Tensor:
    ...
    if bool(selected.any()):
        deformation = gaussians.gaussian_deformation(
            torch.tensor(float(normalized_time), dtype=means.dtype, device=means.device),
            selected,
